...
:: Check if include directory is provided and exists
set "INCLUDE_FLAG="
if not "!INCLUDE_DIR!"=="" (
    ...
    echo No include directory specified - compiling without includes.
)
...
for /R "%SHADER_DIR%" %%F in (*.glsl) do (
    ...
    if defined STAGE (
        ...
        "%GLSLC%" -fshader-stage=!STAGE! "!FILE!" -o "!OUT!" !INCLUDE_FLAG!
        if errorlevel 1 (
